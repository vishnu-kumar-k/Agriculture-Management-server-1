...
async function sendEmailVerification(email, verificationCode) {
  try {
    ...
    const info = await transporter.sendMail({
      ...
      <h1 style="color: #4CAF50; text-align: center;">Welcome to CropPredictor</h1>
      <p style="text-align: center;">Thank you for choosing CropPredictor, where we help your crops thrive!</p>
      <hr style="border: none; border-top: 2px solid #4CAF50;">
      <p style="text-align: center;">To get started with precision farming and crop prediction, please enter the following verification code:</p>
      <div style="text-align: center; padding: 20px; background-color: #f5f5f5; border-radius: 10px;">
          <h2 style="color: #4CAF50; margin: 0;">${verificationCode}</h2>
      </div>
      <!-- Please do not reply to this message. This is an automated notification for your CropPredictor account. -->
  </div>
